...
/**
 * Validates cryptographic JWT Bearer tokens at the API Gateway level using HMAC-SHA256 (HS256).
 */
@Component
    ...
    public boolean validateToken(String token) {
        try {
            ...
            if (!MessageDigest.isEqual(parts[2].getBytes(StandardCharsets.UTF_8), expectedSignature.getBytes(StandardCharsets.UTF_8))) {
                log.warn("Gateway: JWT signature mismatch");
                return false;
            }
            ...
            if (exp != null && Instant.now().getEpochSecond() > exp.longValue()) {
                log.warn("Gateway: JWT token expired");
                return false;
            }

            return true;
        } catch (Exception e) {
            log.warn("Gateway: JWT validation error: {}", e.getMessage());
            return false;
        }
    }
